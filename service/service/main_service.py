...
class LowerService:
    ...
    def executar_rest(self, texts):
        ...
        logger.debug(f"Fim de todas as encriptacoes em {time.time()-start_time}")

        df_response = pd.DataFrame(texts, columns=["textoMensagem"])
        df_response["encriptado"] = response_predicts

        df_response = df_response.drop(columns=["textoMensagem"])

        response = {
            "listafrase": json.loads(
                df_response.to_json(orient="records", force_ascii=False)
            )
        }
